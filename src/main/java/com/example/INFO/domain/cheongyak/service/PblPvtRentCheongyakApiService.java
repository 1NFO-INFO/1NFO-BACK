...
public class PblPvtRentCheongyakApiService {
    ...
    private List<CheongyakDetailsDto> getAptData(int page, int perPage) {
        ...
        CheongyakApiResponse<PblPvtRentCheongyakDetailsResponse> apiResponse = getAptResponse(page, perPage);

        apiResponse.getData().stream()
                .map(PblPvtRentCheongyakDetailsResponse::toDto)
                .forEach(cheongyakDetails::add);

        return cheongyakDetails;
    }

    private CheongyakApiResponse<PblPvtRentCheongyakDetailsResponse> getAptResponse(int page, int perPage) {
        ...
        try {
            return restTemplate.exchange(
                    ...
                    HttpMethod.GET,
                    null,
                    new ParameterizedTypeReference<CheongyakApiResponse<PblPvtRentCheongyakDetailsResponse>>() {}
            ).getBody();
