...
public class UrbtyOfctlCheongyakApiService {
    ...
    private List<CheongyakDetailsDto> getAptData(int page, int perPage) {
        ...
        CheongyakApiResponse<UrbtyOfctlCheongyakDetailsResponse> apiResponse = getAptResponse(page, perPage);

        apiResponse.getData().stream()
                .map(UrbtyOfctlCheongyakDetailsResponse::toDto)
                .forEach(cheongyakDetails::add);

        return cheongyakDetails;
    }

    private CheongyakApiResponse<UrbtyOfctlCheongyakDetailsResponse> getAptResponse(int page, int perPage) {
        ...
        try {
            return restTemplate.exchange(
                    ...
                    HttpMethod.GET,
                    null,
                    new ParameterizedTypeReference<CheongyakApiResponse<UrbtyOfctlCheongyakDetailsResponse>>() {}
            ).getBody();
