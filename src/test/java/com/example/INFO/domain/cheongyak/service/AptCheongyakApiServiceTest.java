...
@SpringBootTest(classes = {AptCheongyakApiService.class, RestTemplateConfig.class})
@TestConstructor(autowireMode = TestConstructor.AutowireMode.ALL)
@RequiredArgsConstructor
@Disabled("공공 데이터 API 호출 테스트 비활성화 (실제 API 호출)")
class AptCheongyakApiServiceTest {
    ...
    void getAllData_DoesNotThrowException() {
        assertDoesNotThrow(cheongyakAptApiService::getAllData);
    }

    @Test
    @DisplayName("api 요청 성공 테스트 - null이 아닌 데이터 반환")
    void getAllData_ReturnNotNull() {
        List<CheongyakDetailsDto> allData = cheongyakAptApiService.getAllData();
